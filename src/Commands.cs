...
#nullable disable
		{
			...
			if (players.Count > 0)
			{
				...
				if (!(AW.g_ButtonBannedPlayer.ContainsKey(targetOnline) ? true : AW.g_ButtonBannedPlayer.TryAdd(targetOnline, new ActBanPlayer(true))))
				{
...
#nullable disable
		{
			...
			ActBanPlayer target = new ActBanPlayer(true);
			...
			if (bOnline)
			{
				...
				if (!(AW.g_ButtonBannedPlayer.ContainsKey(targetController) ? true : AW.g_ButtonBannedPlayer.TryAdd(targetController, new ActBanPlayer(true))))
				{
					UI.ReplyToCommand(admin, bConsole, "Info.Error", "Player not found in dictionary");
					return;
				}
			...
			Server.NextFrame(async () =>
			{
				...
				if (admin != null && target.sAdminSteamID.CompareTo(AW.ConvertSteamID64ToSteamID(admin.SteamID.ToString())) != 0 && !AdminManager.PlayerHasPermissions(admin, "@css/bw_unban_other"))
				{
...
#nullable disable
		{
			...
			if (target == null || !(AW.g_ButtonBannedPlayer.ContainsKey(target) ? true : AW.g_ButtonBannedPlayer.TryAdd(target, new ActBanPlayer(true))))
			{
...
#nullable disable
		{
			...
			{
				if ((AW.g_ButtonBannedPlayer.ContainsKey(target) ? true : AW.g_ButtonBannedPlayer.TryAdd(target, new ActBanPlayer(true))) && AW.g_ButtonBannedPlayer[target].bBanned)
				{
...
#nullable disable
		{
			...
			if (players.Count > 0)
			{
				...
				if (!(AW.g_TriggerBannedPlayer.ContainsKey(targetOnline) ? true : AW.g_TriggerBannedPlayer.TryAdd(targetOnline, new ActBanPlayer(true))))
				{
					UI.ReplyToCommand(admin, bConsole, "Info.Error", "Player not found in dictionary");
					return;
				}
...
#nullable disable
		{
			...
			ActBanPlayer target = new ActBanPlayer(false);
			string sTarget = command.GetArg(1);
			...
			if (bOnline)
			{
				...
				if (!(AW.g_TriggerBannedPlayer.ContainsKey(targetController) ? true : AW.g_TriggerBannedPlayer.TryAdd(targetController, new ActBanPlayer(true))))
				{
					UI.ReplyToCommand(admin, bConsole, "Info.Error", "Player not found in dictionary");
					return;
				}
			...
			Server.NextFrame(async () =>
			{
				...
				if (admin != null && target.sAdminSteamID.CompareTo(AW.ConvertSteamID64ToSteamID(admin.SteamID.ToString())) != 0 && !AdminManager.PlayerHasPermissions(admin, "@css/tw_unban_other"))
				{
					UI.ReplyToCommand(admin, bConsole, "Reply.Ban.Access.Other");
					return;
				}
...
#nullable disable
		{
			...
			if (target == null || !(AW.g_TriggerBannedPlayer.ContainsKey(target) ? true : AW.g_TriggerBannedPlayer.TryAdd(target, new ActBanPlayer(true))))
			{
				UI.ReplyToCommand(player, bConsole, "Info.Error", "Player not found in dictionary");
				return;
			}
...
#nullable disable
		{
			...
			Utilities.GetPlayers().Where(p => p is { IsValid: true, IsBot: false, IsHLTV: false }).ToList().ForEach(target =>
			{
				if ((AW.g_TriggerBannedPlayer.ContainsKey(target) ? true : AW.g_TriggerBannedPlayer.TryAdd(target, new ActBanPlayer(true))) && AW.g_TriggerBannedPlayer[target].bBanned)
				{
					UI.ReplyToCommandMessage(admin, bConsole, $"{AW.g_CFG.color_warning}***{Strlocalizer["Reply.Player"]} {UI.PlayerInfo(target)}{AW.g_CFG.color_warning}***");
